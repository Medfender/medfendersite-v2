...
export default function PresetCard({ preset }: PresetCardProps) {
  ...
  const isDemoPlaying = (id: string) => {
    return (
      currentTrack?.src ===
        preset.demoClips?.find((c) => c.id === id)?.src
    );
  };
  ...
  return (
    <article
      className="group bg-neutral-900/40 border border-white/10 backdrop-blur-md rounded-2xl p-6 transition hover:border-cyan-500/50 flex flex-col justify-between"
    >
      <div>
          ...
          <div className="relative bg-neutral-900/95 border border-white/10 rounded-3xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {(
                  preset.demoClips ||
                  [{ id: "fallback", label: "Main Demo", src: preset.demoAudio }]
                ).map((clip) => {
            ...
            {/* TONEX Captures */}
            {preset.packContents?.tonexCaptures?.length > 0 && (
              <div className="mb-6">
            ...
            {/* IRs */}
            {preset.packContents?.irs?.length > 0 && (
              <div className="mb-6">
            ...
            {/* Signal Chain */}
            {preset.packContents?.signalChain?.length > 0 && (
              <div className="mb-6">
                ...
                <div className="bg-neutral-950/60 rounded-xl p-4">
                  <div className="flex flex-col gap-2 text-sm text-neutral-300">
                    {preset.packContents.signalChain.map((block, idx) => (
